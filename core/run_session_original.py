...
class run_session:
    ...
    def start_adam_optimizer(self):
        ...
        while True:
            ...
            #learning_rate = float(self.conv.rate) 
            ...
            if self.sys_para.traj and self.do_all:
                ...
                self.rl,self.grad_pack, self.norms,self.l,inter_vecs,all_norms= self.session.run(
                [self.tfs.reg_loss,self.tfs.grad_pack,self.tfs.norms,self.tfs.loss,self.tfs.inter_vecs,self.tfs.all_norms], feed_dict=self.feed_dict)
            ...
            if self.sys_para.traj and not self.do_all:
                ...
                max_traj = int(400000/len(self.sys_para.H0))
                #max_traj = 1
                ...
                for kk in range (num_psi0):
                    ...
                    self.grad_pack, self.norms,self.loss,self.rloss,inter_vecs,all_norms = self.session.run(
                [self.tfs.grad_pack,self.tfs.norms,self.tfs.loss,self.tfs.reg_loss, self.tfs.inter_vecs,self.tfs.all_norms], feed_dict=self.feed_dict)
                    #print (all_norms)
                    self.other_l = self.rloss - self.loss
                    ...
                    if w>1:
                        w = 1.0
                    needed_traj[kk] = self.traj_num-np.rint(w* self.traj_num) 
                    if self.norms >1:
                        start[kk] = 1.0
                    else:
                        start[kk] = self.norms
                ...
                for ii in range (len(self.divided_branches)):
                    ...
                    if jump_traj >0:
                        ...
                        self.l = self.l + ((1 - self.norms) * avg_jump_loss / num_psi0) 

                        self.grad_av = self.grad_av + ((1 - self.norms) * self.grad_pack/ num_psi0)  
                    if ii == 0:
                ...
                l,rl,self.g_squared,self.metric, final, target= self.session.run([self.tfs.loss, self.tfs.reg_loss,self.tfs.grad_squared,self.tfs.unitary_scale,self.tfs.final_state,self.tfs.target_vecs], feed_dict=self.feed_dict)
                self.rl = self.l + self.other_l
    ...
    def create_avg_inter_vecs(self):
        ...
        for kk in range (len(self.sys_para.initial_vectors)):
            ...
            if np.sum(self.needed) >0:
                ...
                inter_vecs_mag_squared_list.append(inter_vec_mag_squared_no_jump)
    ...
    def save_data(self):
        if self.sys_para.save:
            ...
            with H5File(self.sys_para.file_path) as hf:
                ...
                hf.append('j', np.array(self.j))
                hf.append('norm',np.array(self.norms))
    ...
    def display(self):
        ...
        if self.show_plots:
            #print (self.j)
            self.conv.update_plot_summary(self.l, self.rl, self.anly,self.g_squared,self.j)
        else:
