...
class TensorflowState:
        ...
        if self.sys_para.state_transfer == False:
            ...
            if self.sys_para.expect:
                if self.sys_para.do_all:
                    ...
                    self.loss = - (self.Il*self.Il)/self.v
    ...
    def All_Trajectories(self,psi0):
        ...
        for ii in np.arange(0,self.sys_para.steps):
            ...
            if self.sys_para.expect:
                ...
                self.all_expects.append(self.expect2(self.expect_op, self.new_psi))
        ...
        if self.sys_para.expect:
            if self.sys_para.do_all:
                ...
                self.all_expectations = tf.stack(self.all_expects, axis=1)
                self.s_e = tf.reduce_sum(self.all_expectations[0,:,0,:],0)
                mean, self.var_e = tf.nn.moments(self.s_e,axes=[0])
                self.s_g = tf.reduce_sum(self.all_expectations[1,:,0,:],0)
                mean, self.var_g = tf.nn.moments(self.s_g,axes=[0])
                self.v = self.var_e + self.var_g
            else:
    ...
    def get_avgd_inner_product2 (self, psi1, psi2, start, end):
        state_num=self.sys_para.state_num
    ...
    def expect2 (self, op, psis):
        result = []
        psis2 = tf.matmul(tf.cast(op,tf.float32),psis)
        if self.num_trajs[0] !=0:
            
            expect1 = self.get_avgd_inner_product2 (psis, psis2, 0, self.num_trajs[0])
            if not self.sys_para.do_all:
                result.append(expect1)
        else:
            expect1 = 0
        if self.num_trajs[1] !=0:
            expect2 = self.get_avgd_inner_product2 (psis, psis2, self.num_trajs[0], self.num_trajs[0] + self.num_trajs[1])
            if not self.sys_para.do_all:
                result.append(expect2)
        else:
            expect2 = 0
